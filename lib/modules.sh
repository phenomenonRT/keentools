...
# скриптов: install.sh uninstall.sh start.sh stop.sh restart.sh update.sh
# status.sh, а также опциональные: check.sh configure.sh log.sh menu.sh
...
# Есть ли у модуля данный опциональный/обязательный скрипт жизненного цикла
kt_module_has_script() {
    [ -f "$(kt_module_dir "$1")/$2" ]
}
...
# Состояние установки.
#
# Версия по-прежнему кэшируется в $KT_STATE_DIR/<module>/installed —
# это нужно для сравнения версий при автообновлении и не требует
# опроса модуля каждый раз.
#
# Но сам факт "установлен ли модуль" теперь, если у модуля есть
# исполняемый check.sh, определяется РЕАЛЬНОЙ проверкой — запуском
# check.sh (например: наличие пакета/бинаря/init.d-скрипта), а не
# просто наличием файла-метки в каталоге KeenTools. Модуль без
# check.sh продолжает работать по-старому — по файлу-метке (это
# нужно для обратной совместимости и для модулей, которые пишут
# только своё внутреннее состояние в data/).
# ---------------------------------------------------------------------------
kt_state_file() { echo "$KT_STATE_DIR/$1/installed"; }

kt_module_is_installed() {
    name="$1"
    if kt_module_has_script "$name" check.sh; then
        kt_module_run "$name" check.sh >/dev/null 2>&1
        return $?
    fi
    [ -f "$(kt_state_file "$name")" ]
}
